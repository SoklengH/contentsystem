                        ...
                        <div class="table-responsive ">
                            <table class="table">
                                <div class="panel panel-default">
                                    <div class="panel-heading">Edit Supplier: {{ $supplier->name }}</div>
                                    ...
                                    <div class="panel-body">
                                        <form action="{{ route('supplier.update', ['id' => $supplier->id]) }}" method="post" enctype="multipart/form-data">
                                            ...
                                            <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                                                ...
                                                <input type="text" name="name" value="{{ $supplier->name }}" class="form-control" required autofocus>
                                            </div>
                                            ...
                                            <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                                                ...
                                                <textarea name="content" id="content" cols="5" rows="5" class="form-control" required autofocus>{{ $supplier->description }}</textarea>
                                            </div>

                                            <div class="form-group">
                                                <div class="text-center">
                                                    <button class="btn btn-success" type="submit">Update Supplier</button>
                                                    <a href="{{ route('suppliers') }}"><button id="supplier" type="button" class="btn btn-success set-botton" style="height: 33px; width: 80px; margin-left: 20px">Back Nah</button></a>
                                                </div>
                                            </div>
                                        </form>
                                    </div>
